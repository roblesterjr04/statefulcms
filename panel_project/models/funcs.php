...
function resultBlock($errors,$successes,$echo=true){
	//Error block
	$output = '';
	$output .= "<div>";
	if(count($errors) > 0)
	{
		...
		{
			$output .= "<p class='bg-danger' style='padding: 15px;'>".$error."</p>";
		}
	}
	//Success block
	if(count($successes) > 0)
	{
		...
		{
			$output .= "<p class='bg-success' style='padding: 15px;'>".$success."</p>";
		}
	}
	$output .= "</div>";
...
function logActivity($summary, $message) {
	apply_filter('logActivity', array("summary"=>$summary, "message"=>$message), true, 1);
	perform_actions('logActivity', array("summary"=>$summary, "message"=>$message));
	global $mysqli;
	$stmt = $mysqli->prepare("INSERT INTO ap_activity_log (message, datestamp, summary) values(?, NOW(), ?)");
	$stmt->bind_param("ss", $message, $summary);
	$stmt->execute();
	$stmt->close();
}

//Retrieve information for all users
function fetchAllUsers($account = NULL)
{
	global $loggedInUser;
	$isadmin = has_permission('Administrator');
	if (!$isadmin && !$account) $account = $loggedInUser->accountid;
	if ($account) $limit_condition = " where account_number = '$account'";
	global $mysqli,$db_table_prefix; 
	$stmt = $mysqli->prepare("SELECT 
		...
		last_sign_in_stamp,
		account_number
		FROM ".$db_table_prefix."users" . $limit_condition
		);
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $account_number);
	
	while ($stmt->fetch()){
		$row[] = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'accountid' => $account_number);
	}
	$stmt->close();
	return ($row);
}
...
function fetchUserDetails($username=NULL,$token=NULL, $id=NULL, $email=NULL)
{
	...
	$stmt->execute();
	$stmt->bind_result($id, $user, $display, $password, $email, $token, $activationRequest, $passwordRequest, $active, $title, $signUp, $signIn, $accountid, $enabled, $image);
	while ($stmt->fetch()){
		$row = array('id' => $id, 'user_name' => $user, 'display_name' => $display, 'password' => $password, 'email' => $email, 'activation_token' => $token, 'last_activation_request' => $activationRequest, 'lost_password_request' => $passwordRequest, 'active' => $active, 'title' => $title, 'sign_up_stamp' => $signUp, 'last_sign_in_stamp' => $signIn, 'account_number' => $accountid, "enabled"=>$enabled, "profile_image"=>$image);
	}
	$stmt->close();
	$perm_list = array();
	$perm_sel = "SELECT
		p.name
		from ".$db_table_prefix."user_permission_matches up
		JOIN ".$db_table_prefix."permissions p on p.id = up.permission_id
		WHERE up.user_id = ?";
	$perm_stmt = $mysqli->prepare($perm_sel);
	$perm_stmt->bind_param("i", $row['id']);
	$perm_stmt->execute();
	$perm_stmt->bind_result($perms);
	while ($perm_stmt->fetch()) {
		$perm_list[] = $perms;
	}
	$perm_stmt->close();
	$row['permissions'] = $perm_list;
	$row = apply_filter('fetchUserDetails', $row);
	return ($row);
}
...
function setUserEnabled($user, $state)
{
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
		SET user_enabled = ?
...
function updateAccountId($id, $accountid)
{
	...
	$stmt = $mysqli->prepare("UPDATE ".$db_table_prefix."users
		...
		id = ?
		LIMIT 1");
	$stmt->bind_param("si", $accountid, $id);
	$result = $stmt->execute();
	$stmt->close();
	return $result;
}
...
function nextAccountId() {
	global $mysqli,$db_table_prefix;
	$stmt = $mysqli->prepare("SELECT max(account_number) from ".$db_table_prefix."users");
	$stmt->execute();
...
function has_permission($permission) {
	global $loggedInUser;
	$perms = $loggedInUser->permissions;
	if ($perms) return in_array($permission, $perms) || in_array('Administrator', $perms);
	else return false;
}
...
function securePage($uri){
	...
	$stmt = $mysqli->prepare("SELECT 
		...
		page = ?
		LIMIT 1");
	$stmt->bind_param("s", basename($uri));
	$stmt->execute();
	$stmt->bind_result($id, $page, $private);
	while ($stmt->fetch()){
